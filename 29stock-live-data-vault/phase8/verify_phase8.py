...
def main():
    ...
    with connect() as conn:
        ...
        conn.commit()
        print('  persisted raw/candle/indicator/feature/derivative rows')

        # No historical overwrite: duplicate minute must not replace the original payload.
        ...
        print('GATE 2 no-overwrite protection: PASS')

        # Timestamp/symbol indexes.
        ...
        print('GATE 3 timestamp/symbol indexing: PASS')

    # Recovery/persistence: a fresh independent DB connection sees committed rows.
    with connect() as recovery:
        ...
        print('GATE 4 connection/process recovery persistence: PASS')
        cleanup(recovery)

    # Archive-before-delete guard: create, checksum, reopen/verify, persist manifest, then delete.
    with tempfile.TemporaryDirectory() as d:
        archive = Path(d) / f'{DATE}.bin'
        archive.write_bytes(b'PSY29 verified daily archive')
        manifest = Path(d) / 'manifest.json'
        digest = sha256_file(archive)
        manifest.write_text(json.dumps({'trading_date':DATE,'sha256':digest,'byte_size':archive.stat().st_size}))
        assert archive_verified(manifest, archive, DATE)
        print('GATE 5 archive checksum/manifest verification: PASS')

    # Verify deletion gate refuses unverified manifests; verified archive deletion is safe.
    class GuardConn:
