...
def stage5(out:Path,syms:list[str],stamp:str):
    ...
    rows=[{"symbol":s,"canonical_rank":int(b[s]["rank"]),"timestamp":stamp,"provenance":"config/psy29_edge_profiles_v1.json","research_provenance":"config/psy29_edge_profiles_v1.json"} for s in syms]
...
def freeze_history(dst:Path):
    """Freeze previous-cycle Stage 17 history before current-cycle execution."""
    purge_invalid_history()
    shutil.rmtree(dst,ignore_errors=True)
    dst.mkdir(parents=True,exist_ok=True)
    previous=sorted(HISTORY.glob("*.csv"))
    for src in previous:shutil.copy2(src,dst/src.name)
    return len(previous)

def append_committed_history(src:Path,stamp:str):
    """Commit current Stage 17 state only after the complete cycle succeeds."""
    HISTORY.mkdir(parents=True,exist_ok=True)
    target=HISTORY/f"stage17_{datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')}.csv"
    write_history_snapshot(src,target,stamp)
    return target
...
def main():
    ...
    if validation.get("status")!="PASS" or validation.get("mode")!="live" or validation.get("live_data") is not True or validation.get("coverage")!={"expected":29,"actual":29,"unique":29}:raise RuntimeError("live pipeline input is not a valid 29/29 DHAN PASS")
    ...
    frozen_history=work/"frozen_stage17_history";history_depth_before=freeze_history(frozen_history)
    ...
    s6=d[6]/"PSY29_STAGE6_LIVE_REGIMES.csv";run([ROOT/"scripts/step9_live_regime_v2.py","--universe",U,"--profiles",P,"--security-map",smap,"--snapshot",snap,"--output",d[6]]);stamp_provenance(s6,6,"PSY29 Stage 6 Live Regime Engine V2")
    s7=d[7]/"PSY29_STAGE7_EDGE_ACTIVATION.csv";run([ROOT/"scripts/stage7_edge_activation.py","--universe",U,"--profiles",P,"--contract",C[7],"--regimes",s6,"--output",d[7]]);stamp_provenance(s7,7,"PSY29 Stage 7 Research-Conditioned Edge Activation")
    s8=d[8]/"PSY29_STAGE8_EDGE_RANKING.csv";run([ROOT/"scripts/stage8_edge_ranking.py","--universe",U,"--profiles",P,"--contract",C[8],"--edge-activation",s7,"--regimes",s6,"--output",d[8]]);stamp_provenance(s8,8,"PSY29 Stage 8 Portfolio Edge Ranking")
    s9=d[9]/"PSY29_STAGE9_CANDIDATE_QUALITY.csv";run([ROOT/"scripts/stage9_candidate_quality.py","--universe",U,"--profiles",P,"--contract",C[9],"--stage6",s6,"--stage7",s7,"--stage8",s8,"--output",d[9]]);stamp_provenance(s9,9,"PSY29 Stage 9 Live Candidate Quality Engine")
    s10=d[10]/"PSY29_STAGE10_INTEGRITY.csv";run([ROOT/"scripts/stage10_integrity_gate.py","--universe",U,"--profiles",P,"--contract",C[10],"--stage6",s6,"--stage7",s7,"--stage8",s8,"--stage9",s9,"--output",d[10]]);stamp_provenance(s10,10,"PSY29 Stage 10 Integrity Gate")
    s11=d[11]/"PSY29_STAGE11_EXECUTION_ANALYSIS.csv";run([ROOT/"scripts/stage11_execution_analysis.py","--universe",U,"--profiles",P,"--contract",C[11],"--stage6",s6,"--stage7",s7,"--stage8",s8,"--stage9",s9,"--stage10",s10,"--live-snapshot",exe,"--output",d[11]]);s11c=work/"stage11_compat.csv";add_s11_prov(s11,s11c)
    s12=d[12]/"PSY29_STAGE12_EXECUTION_READINESS.csv";run([ROOT/"scripts/stage12_execution_readiness.py","--universe",U,"--profiles",P,"--stage6",s6,"--stage7",s7,"--stage8",s8,"--stage9",s9,"--stage10",s10,"--stage11",s11c,"--contract",C[12],"--output",d[12]]);stamp_provenance(s12,12,"PSY29 Stage 12 Execution Readiness")
    s13=d[13]/"PSY29_STAGE13_SCENARIO_ADJUDICATION.csv";run([ROOT/"scripts/stage13_scenario_adjudication.py","--universe",U,"--profiles",P,"--stage6",s6,"--stage7",s7,"--stage8",s8,"--stage9",s9,"--stage10",s10,"--stage11",s11c,"--stage12",s12,"--contract",C[13],"--output",d[13]]);stamp_provenance(s13,13,"PSY29 Stage 13 Scenario Adjudication")
    s14=d[14]/"PSY29_STAGE14_LIVE_DASHBOARD.csv";run([ROOT/"scripts/stage14_live_dashboard.py","--universe",U,"--profiles",P,"--stage6",s6,"--stage7",s7,"--stage8",s8,"--stage9",s9,"--stage10",s10,"--stage11",s11c,"--stage12",s12,"--stage13",s13,"--contract",C[14],"--output",d[14]]);stamp_provenance(s14,14,"PSY29 Stage 14 Live Dashboard")
    ...
    s16=d[16]/"PSY29_STAGE16_CURRENT_BOARD.csv";run([ROOT/"scripts/stage16_live_decision_consolidation.py","--universe",U,"--stage5",s5,"--stage6",s6,"--stage7",s7,"--stage8",s8,"--stage9",s9,"--stage10",s10,"--stage11",s11c,"--stage12",s12,"--stage13",s13,"--stage14",s14,"--stage15",s15c,"--output",d[16]]);stamp_provenance(s16,16,"PSY29 Stage 16 Decision Consolidation")
    for n,p in ((6,s6),(7,s7),(8,s8),(9,s9),(10,s10),(11,s11),(12,s12),(13,s13),(14,s14),(15,s15),(16,s16)):verify29(p,exp,f"Stage {n}")
    s17=d[17]/"PSY29_STAGE17_STABILITY_BOARD.csv";run([ROOT/"scripts/stage17_live_stability.py","--universe",U,"--stage5",s5,"--stage6",s6,"--stage7",s7,"--stage8",s8,"--stage9",s9,"--stage10",s10,"--stage11",s11c,"--stage12",s12,"--stage13",s13,"--stage14",s14,"--stage15",s15c,"--stage16",s16,"--output",d[17]]);stamp_provenance(s17,17,"PSY29 Stage 17 Live Decision Stability Engine");verify29(s17,exp,"Stage 17")
    current_stage17=work/"current_stage17_pending.csv";write_history_snapshot(s17,current_stage17,stamp)
    s18=d[18]/"PSY29_STAGE18_HISTORICAL_BOARD.csv";run([ROOT/"scripts/stage18_historical_continuity.py","--universe",U,"--stage5",s5,"--stage6",s6,"--stage7",s7,"--stage8",s8,"--stage9",s9,"--stage10",s10,"--stage11",s11c,"--stage12",s12,"--stage13",s13,"--stage14",s14,"--stage15",s15c,"--stage16",s16,"--stage17",s17,"--history",frozen_history,"--output",d[18]]);stamp_provenance(s18,18,"PSY29 Stage 18 Historical Continuity Engine");verify29(s18,exp,"Stage 18")
    s19=d[19]/"PSY29_STAGE19_TRANSITION_BOARD.csv";run([ROOT/"scripts/stage19_forward_transition.py","--contract",C[19],"--universe",U,"--stage17",s17,"--stage18",s18,"--history",frozen_history,"--output",d[19]]);stamp_provenance(s19,19,"PSY29 Stage 19 Forward State Transition Engine");verify29(s19,exp,"Stage 19")
    ...
    append_committed_history(current_stage17,stamp)
    history_depth_after=len(list(HISTORY.glob("*.csv")))
    board=json.loads((out/"PSY29_STAGE20_FINAL_SIGNAL_BOARD.json").read_text(encoding="utf-8"));board["production_cycle"]={"generated_at":stamp,"live_data":True,"provider":"DHAN","stage6_to_stage20":True,"stage17_history_frozen_before_cycle":True,"stage17_history_depth_before_cycle":history_depth_before,"stage17_current_appended_after_cycle_success":True,"history_depth":history_depth_after};(out/"PSY29_STAGE20_FINAL_SIGNAL_BOARD.json").write_text(json.dumps(board,indent=2),encoding="utf-8")
    print(f"PSY29 LIVE SIGNAL CYCLE: PASS | signals={board.get('signal_count',0)} | history_depth={history_depth_after} | previous_history_frozen={history_depth_before}",flush=True)
