...
C={n:ROOT/f"config/psy29_stage{n}_{name}.json" for n,name in {7:"edge_activation_contract",8:"edge_ranking_contract",9:"candidate_quality_contract",10:"integrity_gate_contract",11:"execution_analysis_contract",12:"execution_readiness_contract",13:"scenario_adjudication_contract",14:"live_dashboard_contract"}.items()}
...
def main():
    ...
    snap=work/"snapshot.csv";shutil.copy2(exe,snap)
    raw=csv_rows(exe);smap=work/"security_map.json"
    ...
    history_file=HISTORY/f"stage17_{datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')}.csv";shutil.copy2(s17,history_file)
    s18=d[18]/"PSY29_STAGE18_HISTORICAL_BOARD.csv";run([ROOT/"scripts/stage18_historical_continuity.py","--universe",U,"--stage5",s5,"--stage6",s6,"--stage7",s7,"--stage8",s8,"--stage9",s9,"--stage10",s10,"--stage11",s11c,"--stage12",s12,"--stage13",s13,"--stage14",s14,"--stage15",s15,"--stage16",s16,"--stage17",s17,"--history",HISTORY,"--output",d[18]]);s18=d[18]/"PSY29_STAGE18_HISTORICAL_BOARD.csv";verify29(s18,exp,"Stage 18")
    s19=d[19]/"PSY29_STAGE19_TRANSITION_BOARD.csv";run([ROOT/"scripts/stage19_forward_transition.py","--contract",C[19],"--universe",U,"--stage17",s17,"--stage18",s18,"--history",HISTORY,"--output",d[19]]);verify29(s19,exp,"Stage 19")
    memory=out/"PSY29_STAGE20_SIGNAL_MEMORY.json"
    stage20=out/"stage20";shutil.rmtree(stage20,ignore_errors=True);stage20.mkdir(parents=True)
