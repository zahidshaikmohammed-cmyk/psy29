...
ROOT=Path(__file__).resolve().parents[1];OUT=ROOT/"runtime/live";U=ROOT/"config/psy29_live_universe_contract.json";P=ROOT/"config/psy29_edge_profiles_v1.json";C={n:ROOT/f"config/psy29_stage{n}_{name}.json" for n,name in {7:"edge_activation_contract",8:"edge_ranking_contract",9:"candidate_quality_contract",10:"integrity_gate_contract",11:"execution_analysis_contract",12:"execution_readiness_contract",13:"scenario_adjudication_contract",14:"live_dashboard_contract"}.items()};C[19]=ROOT/"config/psy29_stage19_forward_state_transition_contract.json";HISTORY=OUT/"stage17_history"
def run(cmd:list[object],timeout:int=300):
    print("PSY29 SIGNAL:"," ".join(map(str,cmd)),flush=True);subprocess.run([sys.executable,*map(str,cmd)],cwd=ROOT,check=True,timeout=timeout)
def csv_rows(p:Path):
...
def stage15_universe_view(out:Path,syms:list[str]):out.write_text(json.dumps({"symbols":syms},indent=2),encoding="utf-8")
def write_history_snapshot(src:Path,dst:Path,stamp:str):
    ...
    with dst.open("w",encoding="utf-8",newline="") as f:w=csv.DictWriter(f,fieldnames=fields);w.writeheader();w.writerows(rows)
...
def main():
    ap=argparse.ArgumentParser();ap.add_argument("--output",type=Path,default=OUT);a=ap.parse_args();out=a.output;out.mkdir(parents=True,exist_ok=True)
    ...
    verify29(exe,exp,"Live execution snapshot");stamp=datetime.now(timezone.utc).replace(microsecond=0).isoformat().replace("+00:00","Z")
    work=out/"signal_cycle";shutil.rmtree(work,ignore_errors=True);work.mkdir(parents=True);d={n:work/f"stage{n}" for n in range(6,20)}
    for x in d.values():x.mkdir(parents=True,exist_ok=True)
    snap=work/"snapshot.csv";shutil.copy2(exe,snap);raw=csv_rows(exe);smap=work/"security_map.json";mappings=[{"symbol":s,"security_id":str(next(r for r in raw if str(r["symbol"]).upper().strip()==s).get("security_id","")).strip(),"exchange":"NSE","segment":"E"} for s in syms]
