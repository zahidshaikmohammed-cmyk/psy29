...
def main():
    ...
    if c.get("strategy_authority",{}).get("generic_rr_fallback_forbidden") is not True:raise ValueError("generic RR fallback must remain forbidden")
    syms=universe(a.universe);expected=set(syms);s11=index(a.stage11,expected,"Stage 11");s16=index(a.stage16,expected,"Stage 16");s19=index(a.stage19,expected,"Stage 19");mem=memory(a.memory,expected)
    now=datetime.now(timezone.utc);stamp=now.replace(microsecond=0).isoformat().replace("+00:00","Z");signals=[];audit=[]
    for rank,s in enumerate(syms,1):
        ...
        ok11,_,why11=fresh(r11,now);ok16,_,why16=fresh(r16,now);ok19,_,why19=fresh(r19,now)
        prov11=bool(val(r11,"stage11_provenance","provenance","research_provenance"));prov16=bool(val(r16,"stage16_provenance","provenance"));prov19=bool(val(r19,"stage19_provenance","provenance"))
        if not (ok11 and ok16 and ok19):reason=f"freshness: S11={why11}; S16={why16}; S19={why19}"
            ...
            if conf is None or not 0<=conf<=1:reason="scenario confidence missing or outside 0..1"
                ...
                entry,stop,target,rr=levels;score=round(conf*100.0,4);seed=f"{s}|{sc}|{entry:.6f}|{stop:.6f}|{target:.6f}|{stamp}";sid="PSY29-"+hashlib.sha256(seed.encode()).hexdigest()[:16].upper()
                signals.append({"signal_id":sid,"symbol":s,"canonical_rank":rank,"direction":d,"strategy":sc,"entry":entry,"stop_loss":stop,"take_profit":target,"risk_reward":round(rr,4),"signal_score":score,"signal_status":"NEW_SIGNAL","timestamp":stamp,"memory_state":m,"stage16_state":state,"stage19_state":trans,"provenance":"PSY29 Stage 20 strategy-owned signal; Stage 11 live levels + Stage 16 candidate + Stage 19 transition"});reason="qualifying new strategy-owned opportunity";decision="SIGNAL"
        audit.append({"symbol":s,"stage16_state":state,"stage19_state":trans,"scenario":sc,"memory_state":m,"decision":decision,"reason":reason})
    payload={"stage":20,"version":"1.0","status":"PASS","generated_at":stamp,"coverage":{"expected":29,"actual":29,"unique":29},"signal_count":len(signals),"signals":signals,"audit":audit,"daily_signal_cap":None,"provenance":"PSY29 Stage 20 Final Trading Signal & Strategy Engine v1.0"}
    ...
    (a.output/"PSY29_STAGE20_SIGNAL_MEMORY.json").write_text(json.dumps({"stage":20,"updated_at":stamp,"records":[{"symbol":s,"state":("ACTIVE_SIGNAL" if any(x["symbol"]==s for x in signals) else mem[s])} for s in syms]},indent=2),encoding="utf-8")
    validation={"stage":20,"version":"1.0","validation_status":"PASS","canonical_29":True,"multiple_signals_allowed":True,"signal_count":len(signals),"daily_cap":None,"strategy_owned_levels":all(all(k in x for k in ("entry","stop_loss","take_profit","strategy")) for x in signals),"no_generic_fallback":True,"freshness_gate":True,"provenance_gate":True,"blocked_execution_fields":[],"fail_closed":True};(a.output/"PSY29_STAGE20_VALIDATION.json").write_text(json.dumps(validation,indent=2),encoding="utf-8")
    print("PSY29 STAGE 20: PASS");print("Canonical 29/29: PASS");print(f"Signals emitted: {len(signals)}");print("Strategy-owned Entry/SL/TP: PASS");print("Freshness + provenance gates: PASS");print("No daily signal cap: PASS")
