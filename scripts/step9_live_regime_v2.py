...
def canonical_symbols(universe: dict[str, Any]) -> list[str]:
    ...
    if len(symbols) != 29:
        raise ValueError(
            f"Canonical universe must contain exactly 29 symbols; got {len(symbols)}"
        )
...
def validate_profiles(
    ...
    if len(rows) != 29:
        raise ValueError(
            f"Stage 5 profile count must be 29; got {len(rows)}"
        )
    ...
    if set(profile_symbols) != set(symbols):
        ...
        raise ValueError(
            f"Stage 5 profile mismatch; missing={missing}, unexpected={unexpected}"
        )


def validate_security_map(
    ...
    if len(rows) != 29:
        raise ValueError(
            f"Security-ID map must contain exactly 29 mappings; got {len(rows)}"
        )
    ...
    if set(mapping) != set(symbols):
        ...
        raise ValueError(
            f"Security-ID universe mismatch; missing={missing}, unexpected={unexpected}"
        )
...
def data_age_seconds(value: Any, now: datetime) -> float:
    ...
    # Future timestamps are invalid rather than treated as fresh.
    if age < -5:
...
def classify_regime(row: pd.Series) -> str:
    ...
    No trade authorization occurs here.
    """
    ...
    # Directional alignment without opening-range confirmation.
    if bullish_score == 2:
    ...
    # One directional component only.
...
def validate_snapshot_columns(snapshot: pd.DataFrame) -> None:
    ...
    if missing:
        raise ValueError(
            f"Live snapshot missing required fields: {sorted(missing)}"
        )
...
def process(
    ...
    resolved_security_ids = validate_security_map(
        security_map,
        symbols,
    )
    ...
    # Unknown symbols are rejected rather than silently ignored.
    unknown_symbols = sorted(set(snapshot["symbol"]) - set(symbols))

    if unknown_symbols:
        raise ValueError(
            f"Live snapshot contains symbols outside canonical 29: {unknown_symbols}"
        )

    # Duplicate snapshots are resolved deterministically to the latest row.
    ...
    profile_by_symbol = {
        str(row["symbol"]).upper().strip(): row
    ...
    output.to_csv(
        out / "PSY29_STAGE6_LIVE_REGIMES.csv",
        index=False,
    )
    ...
    summary = {
        ...
        "regime_counts": {
            regime: int((output["regime"] == regime).sum())
...
def main() -> None:
    ...
    summary = process(
        ...
        security_map_path=args.security_map,
