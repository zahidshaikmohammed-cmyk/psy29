...
def build_execution_row(symbol:str,security_id:str,x1:pd.DataFrame,x5:pd.DataFrame,timestamp:str)->dict:
    ...
    return {"symbol":symbol,"security_id":security_id,"exchange_segment":"NSE_EQ","timestamp":timestamp,"open_1m":float(latest["open"]),"high_1m":float(latest["high"]),"low_1m":float(latest["low"]),"close_1m":float(latest["close"]),"volume_1m":float(latest["volume"]),"avg_volume_20_1m":float(x1["volume"].tail(20).mean()),"open_5m":float(b5["open"]),"high_5m":float(b5["high"]),"low_5m":float(b5["low"]),"close_5m":float(b5["close"]),"volume_5m":float(b5["volume"]),"avg_volume_20_5m":float(x5["volume"].tail(20).mean()),"vwap_5m":float(latest5["vwap5"]),"ema9_5m":float(latest5["ema9_5m"]),"ema20_5m":float(latest5["ema20_5m"]),"first15_high":float(latest["first15_high"]),"first15_low":float(latest["first15_low"]),"swing_high":float(recent["high"].max()),"swing_low":float(recent["low"].min()),"freshness_status":"FRESH"}
def main():
