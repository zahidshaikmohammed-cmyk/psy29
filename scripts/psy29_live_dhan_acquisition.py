...
IST=ZoneInfo("Asia/Kolkata");MASTER_URL="https://images.dhan.co/api-data/api-scrip-master-detailed.csv";FRESH_MAX_AGE_SECONDS=120;WARMUP_TRADING_SESSIONS=5;REQUEST_SPACING_SECONDS=0.22;STRUCTURAL_SWING_BARS=20;SESSION_OPEN=dtime(9,15);SESSION_CLOSE=dtime(15,30);OPENING_RANGE_END=dtime(9,30)
...
def _warmup_start(now):
 """Return the start of the fifth most-recent trading session, inclusive.

 Dhan's intraday endpoint is bounded to a small recent-session window. The old
 implementation requested ten calendar days, which can cross that provider
 boundary and produce HTTP 400 for every symbol. Keep every request inside the
 latest five NSE trading sessions and retain enough bars for EMA/volume warmup.
 """
 d=now.date();seen=0
 while True:
  if d.weekday()<5:
   seen+=1
   if seen>=WARMUP_TRADING_SESSIONS:return datetime.combine(d,SESSION_OPEN,tzinfo=IST)
  d-=timedelta(days=1)
def fetch_bars(client,security_id,now,interval):
 start=_warmup_start(now);p={"securityId":str(security_id),"exchangeSegment":"NSE_EQ","instrument":"EQUITY","interval":interval,"oi":False,"fromDate":start.strftime("%Y-%m-%d %H:%M:%S"),"toDate":now.strftime("%Y-%m-%d %H:%M:%S")}
 data=client.post("/charts/intraday",p,timeout=20,retries=2)
...
def main():
 ...
 for s in symbols:
  try:
   ...
   snap.append({"symbol":s,"candle_start_timestamp":datetime.fromtimestamp(latest_ts,tz=timezone.utc).isoformat().replace("+00:00","Z"),"timestamp":ts,"last_price":float(x1.iloc[-1].close),"vwap":float(x1.iloc[-1].vwap),"ema9":float(x1.iloc[-1].ema9),"ema20":float(x1.iloc[-1].ema20),"first15_high":float(x1.iloc[-1].first15_high),"first15_low":float(x1.iloc[-1].first15_low),"session_high":float(x5.high.max()),"session_low":float(x5.low.min()),"freshness_age_seconds":round(max(0.0,age),3),"freshness_status":"FRESH","provider":"DHAN","security_id":mapping[s],"exchange_segment":"NSE_EQ","indicator_warmup":"5_TRADING_SESSIONS","candle_completion_policy":"COMPLETED_CANDLES_ONLY","opening_range_complete":True,"session_extreme_policy":"CURRENT_SESSION_COMPLETED_5M_RUNNING_EXTREMES"})
 ...
 validation={"contract":"PSY29_LIVE_DHAN_ACQUISITION_VALIDATION","status":status,"provider":"DHAN","live_data":status=="PASS","market_data_kind":"LIVE_INTRADAY_COMPLETED_CANDLES","coverage":{"expected":29,"actual":len(snap),"unique":int(snapshot.symbol.nunique()) if not snapshot.empty else 0},"fresh_count":int((snapshot.freshness_status=="FRESH").sum()) if not snapshot.empty else 0,"stale_count":stale,"invalid_count":invalid,"errors":errors,"timestamp":datetime.now(timezone.utc).isoformat().replace("+00:00","Z"),"source":"PSY29/dhan/client.py + DHAN v2 intraday historical data","indicator_warmup":"5_TRADING_SESSIONS","provider_request_window_policy":"MAX_5_RECENT_TRADING_SESSIONS","request_spacing_seconds":REQUEST_SPACING_SECONDS,"freshness_max_age_seconds":FRESH_MAX_AGE_SECONDS,"freshness_policy":"COMPLETED_1M_CANDLE_END_WITHIN_120_SECONDS","structural_swing_warmup":"PRIOR_COMPLETED_SESSIONS","structural_swing_window_bars":STRUCTURAL_SWING_BARS,"structural_swing_policy":"PREVIOUS_SESSIONS_ONLY_EXCLUDE_CURRENT_SESSION","candle_completion_policy":"COMPLETED_CANDLES_ONLY","freshness_timestamp_policy":"CANDLE_END_TIMESTAMP","opening_range_policy":"09:15-09:29 IST, complete before pipeline acceptance","session_extreme_policy":"CURRENT_SESSION_COMPLETED_5M_RUNNING_EXTREMES","signal_generation":False,"order_execution":False}
 (out/"live_acquisition_validation.json").write_text(json.dumps(validation,indent=2),encoding="utf-8");(out/"security_map.json").write_text(json.dumps({"status":"PASS" if len(mapping)==29 else "FAIL","canonical_count":29,"resolved_count":len(mapping),"unique_security_id_count":len(set(mapping.values())),"mappings":[{"symbol":s,"security_id":mapping[s],"exchange":"NSE","segment":"E","exchange_segment":"NSE_EQ"} for s in symbols if s in mapping]},indent=2),encoding="utf-8");print(json.dumps(validation,indent=2))
