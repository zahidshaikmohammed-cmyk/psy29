...
# When executed as `python scripts/<file>.py`, Python puts `scripts/` rather
# than the repository root on sys.path. Add the PSY29 root explicitly so the
# repository-local `dhan` package is always importable on Render.
...
def fetch_1m(client: DhanClient, security_id: str, now: datetime) -> pd.DataFrame:
    ...
    payload = {"securityId": str(security_id), "exchangeSegment": "NSE_EQ", "instrument": "EQUITY", "interval": "1", "oi": False, "fromDate": start.strftime("%Y-%m-%d %H:%M:%S"), "toDate": now.strftime("%Y-%m-%d %H:%M:%S")}
...
def add_indicators(df: pd.DataFrame) -> pd.DataFrame:
    ...
    x["vwap"] = (typical * vol).cumsum() / vol.cumsum().replace(0, math.nan)
    x["ema9"] = x["close"].ewm(span=9, adjust=False).mean()
    x["ema20"] = x["close"].ewm(span=20, adjust=False).mean()
    opening = x[x["dt"].dt.time <= datetime.strptime("09:29", "%H:%M").time()]
    if opening.empty: opening = x.head(15)
    x["first15_high"], x["first15_low"] = float(opening["high"].max()), float(opening["low"].min())
    return x


def build_execution_row(symbol: str, df: pd.DataFrame, timestamp: str) -> dict:
    if len(df) < 20: raise RuntimeError(f"{symbol}: fewer than 20 live 1m candles")
    x = df.copy(); x["bucket"] = x["dt"].dt.floor("5min")
    bars = x.groupby("bucket", sort=True).agg(open=("open", "first"), high=("high", "max"), low=("low", "min"), close=("close", "last"), volume=("volume", "sum")).reset_index()
    if len(bars) < 20: raise RuntimeError(f"{symbol}: fewer than 20 derived 5m candles")
    latest, b5, recent = x.iloc[-1], bars.iloc[-1], bars.tail(20)
    return {"symbol": symbol, "timestamp": timestamp, "open_1m": float(latest["open"]), "high_1m": float(latest["high"]), "low_1m": float(latest["low"]), "close_1m": float(latest["close"]), "volume_1m": float(latest["volume"]), "avg_volume_20_1m": float(x["volume"].tail(20).mean()), "open_5m": float(b5["open"]), "high_5m": float(b5["high"]), "low_5m": float(b5["low"]), "close_5m": float(b5["close"]), "volume_5m": float(b5["volume"]), "avg_volume_20_5m": float(bars["volume"].tail(20).mean()), "vwap_5m": float(latest["vwap"]), "ema9_5m": float(latest["ema9"]), "ema20_5m": float(latest["ema20"]), "first15_high": float(latest["first15_high"]), "first15_low": float(latest["first15_low"]), "swing_high": float(recent["high"].max()), "swing_low": float(recent["low"].min())}
...
def main() -> None:
    ...
    symbols = load_universe(args.universe); client = DhanClient(); mapping = resolve_security_ids(symbols); now = datetime.now(IST); now_utc = now.astimezone(timezone.utc)
    snapshot_rows, execution_rows, errors = [], [], []
    for symbol in symbols:
        try:
            x = add_indicators(fetch_1m(client, mapping[symbol], now))
            if x.empty: raise RuntimeError("no current-session candles returned")
            latest_ts = int(x.iloc[-1]["timestamp"]); age = (now_utc - datetime.fromtimestamp(latest_ts, tz=timezone.utc)).total_seconds()
            ...
            snapshot_rows.append({"symbol": symbol, "timestamp": ts, "last_price": float(x.iloc[-1]["close"]), "vwap": float(x.iloc[-1]["vwap"]), "ema9": float(x.iloc[-1]["ema9"]), "ema20": float(x.iloc[-1]["ema20"]), "first15_high": float(x.iloc[-1]["first15_high"]), "first15_low": float(x.iloc[-1]["first15_low"]), "freshness_age_seconds": round(max(0.0, age), 3), "freshness_status": "FRESH" if age <= FRESH_MAX_AGE_SECONDS else ("STALE" if age <= STALE_MAX_AGE_SECONDS else "INVALID"), "provider": "DHAN", "security_id": mapping[symbol], "exchange_segment": "NSE_EQ"})
            execution_rows.append(build_execution_row(symbol, x, ts))
