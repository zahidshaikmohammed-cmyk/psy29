...
# Correct the canonical Stage 19 filename used by the live pipeline.
...
def worker():
    while True:
        try:
            with LOCK:
                STATE.update({"status":"RUNNING","error":None});save_runtime_state(RUNTIME_STATE,{"status":"RUNNING","error":None})
            ...
            with LOCK:
                STATE.update({"status":"PASS","error":None,"last_cycle":time.time()});save_runtime_state(RUNTIME_STATE,{"status":"PASS","error":None,"last_cycle":time.time(),"last_cycle_id":cycle_id,"last_signal_count":signals,"last_history_depth":history})
