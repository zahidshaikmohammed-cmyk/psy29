...
STAGE_ARTIFACTS = {
    6: "stage6/PSY29_STAGE6_LIVE_REGIMES.csv",
    7: "stage7/PSY29_STAGE7_EDGE_ACTIVATION.csv",
    8: "stage8/PSY29_STAGE8_EDGE_RANKING.csv",
    9: "stage9/PSY29_STAGE9_CANDIDATE_QUALITY.csv",
    10: "stage10/PSY29_STAGE10_INTEGRITY.csv",
    11: "stage11/PSY29_STAGE11_EXECUTION_ANALYSIS.csv",
    12: "stage12/PSY29_STAGE12_EXECUTION_READINESS.csv",
    13: "stage13/PSY29_STAGE13_SCENARIO_ADJUDICATION.csv",
    14: "stage14/PSY29_STAGE14_LIVE_DASHBOARD.csv",
    15: "stage15/PSY29_STAGE15_EVENTS.csv",
    16: "stage16/PSY29_STAGE16_CURRENT_BOARD.csv",
    17: "stage17/PSY29_STAGE17_STABILITY_BOARD.csv",
    18: "stage18/PSY29_STAGE18_HISTORICAL_BOARD.csv",
    19: "stage19/PSY29_STAGE19_TRANSITION_BOARD.csv",
}
STAGE_NAMES = {
    6: "Regime", 7: "Edge Activation", 8: "Edge Ranking", 9: "Candidate Quality",
    10: "Integrity", 11: "Execution Analysis", 12: "Readiness", 13: "Scenario",
    14: "Dashboard", 15: "Event Journal", 16: "Decision Consolidation", 17: "Stability",
    18: "Continuity", 19: "Transition", 20: "Final Authority",
}
...
def recent_ready():
    validation = rj(RV, {}) or {}
    if validation.get("status") != "PASS" or validation.get("provider") != "DHAN" or validation.get("live_data") is not False:
        return False
...
def audit_for_symbol(symbol):
    ...
    board = rj(BOARD, {}) or {}
    audit = next((x for x in board.get("audit", []) if str(x.get("symbol", "")).strip().upper() == symbol), None)
    stages = []
    for stage, rel in STAGE_ARTIFACTS.items():
        path = CYCLE / rel
        row = row_for_symbol(path, symbol) if path.exists() else None
        stages.append({
            "stage": stage, "name": STAGE_NAMES[stage], "artifact": rel,
            "artifact_exists": path.exists(), "row_present": row is not None,
            "state": row_state(row), "timestamp": row_timestamp(row),
            "provenance_present": provenance_present(row),
            "sha256": sha256(path) if path.exists() else None,
        })
    final_path = BOARD
    stages.append({
        "stage": 20, "name": STAGE_NAMES[20],
        "artifact": str(final_path.relative_to(ROOT)) if final_path.exists() else "runtime/live/PSY29_STAGE20_FINAL_SIGNAL_BOARD.json",
        "artifact_exists": final_path.exists(), "row_present": audit is not None,
        "state": (audit or {}).get("decision") if isinstance(audit, dict) else None,
        "timestamp": board.get("generated_at"),
        "provenance_present": bool(board.get("provenance")),
        "sha256": sha256(final_path) if final_path.exists() else None,
    })
    return {
        "symbol": symbol,
        "cycle_id": board.get("generated_at"),
        "final_decision": (audit or {}).get("decision") if isinstance(audit, dict) else None,
        "final_reason": (audit or {}).get("reason") if isinstance(audit, dict) else None,
        "stage16_state": (audit or {}).get("stage16_state") if isinstance(audit, dict) else None,
        "stage19_state": (audit or {}).get("stage19_state") if isinstance(audit, dict) else None,
        "scenario": (audit or {}).get("scenario") if isinstance(audit, dict) else None,
        "memory_state": (audit or {}).get("memory_state") if isinstance(audit, dict) else None,
        "stages": stages,
    }
...
def signal_data():
    ...
    for item in audit:
        if not isinstance(item, dict):
            continue
        x = dict(item); x["symbol"] = str(x.get("symbol", "")).strip().upper()
        x["signal"] = next((s for s in active if str(s.get("symbol", "")).strip().upper() == x["symbol"]), None)
...
def instrument(symbol):
    symbol = symbol.strip().upper(); validation = rj(PV, {}) or {}; board = rj(BOARD, {}) or {}
    signal = next((row for row in rc(SIGNALS) if str(row.get("symbol", "")).strip().upper() == symbol), None)
    live = validation.get("live_data") is True
    ...
    audit = next((x for x in (board.get("audit") or []) if str(x.get("symbol", "")).strip().upper() == symbol), None)
    if not row:
        return {"symbol": symbol, "found": False, "live_data": live, "message": "Fresh live DHAN cycle is still pending." if market_open_now() and not live else ("Recent DHAN market data is not available yet." if not live else "Live instrument data not available yet."), "condition": audit, "audit": audit_for_symbol(symbol)}
...
def worker():
    while True:
        try:
            ...
            if recent_stale():
                subprocess.run([sys.executable, str(ROOT / "scripts/psy29_recent_dhan_snapshot.py"), "--universe", str(UNIVERSE), "--output", str(OUT)], cwd=ROOT, check=True, timeout=240)
