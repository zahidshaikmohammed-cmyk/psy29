...
def signal_data():
 ...
 return {"service":"PSY29 LIVE SIGNAL BOARD","status":"LIVE" if live else "OFF_MARKET","live_data":live,"signal_generation":False,"order_execution":False,"coverage":v.get("coverage",{"expected":29,"actual":0,"unique":0}),"timestamp":b.get("timestamp") or v.get("timestamp"),"signals":sig,"active_signal_count":len(sig),"message":None if sig else "NO ACTIVE PSY29 SIGNAL"}

def instrument(symbol):
 symbol=symbol.strip().upper(); row=next((x for x in rc(PIPELINE) if str(x.get("symbol","")).strip().upper()==symbol),None); v=rj(PV,{}) or {}; b=rj(BOARD,{}) or {}; sig=next((x for x in rc(SIGNALS) if str(x.get("symbol","")).strip().upper()==symbol),None)
 if not row:return {"symbol":symbol,"found":False,"message":"Instrument data not available yet."}
 keys=["open_1m","high_1m","low_1m","close_1m","volume_1m","open_5m","high_5m","low_5m","close_5m","volume_5m","vwap_5m","ema9_5m","ema20_5m","first15_high","first15_low","swing_high","swing_low","timestamp","freshness_status"]
 return {"symbol":symbol,"found":True,"live_data":v.get("live_data") is True,"coverage":v.get("coverage"),"data":{k:row.get(k) for k in keys},"signal":sig if v.get("live_data") is True else None,"strategy":(sig or {}).get("strategy") if v.get("live_data") is True else None,"stage20_board_status":b.get("status")}
...
async function load(){let s=await j('/api/signals');document.getElementById('status').textContent=s.live_data?'● LIVE DHAN':'OFF-MARKET';if(!universe.length){universe=(await j('/api/universe')).symbols||[];document.getElementById('stocks').innerHTML=universe.map(x=>`<button class="stock" data-s="${esc(x)}" onclick="select('${esc(x)}')"><span><i class="dot"></i>${esc(x)}</span><span class="muted">›</span></button>`).join('')}if(selected)await select(selected);else if(universe[0])await select(universe[0])}
async function select(x){selected=x;document.querySelectorAll('.stock').forEach(b=>b.classList.toggle('active',b.dataset.s===x));let d=await j('/api/instrument/'+encodeURIComponent(x));document.getElementById('title').textContent=x;if(!d.found)return;let q=d.data;document.getElementById('summary').innerHTML=[['Last',q.close_1m],['VWAP',q.vwap_5m],['EMA 9',q.ema9_5m],['EMA 20',q.ema20_5m],['1m Volume',q.volume_1m],['5m Volume',q.volume_5m],['15m High',q.first15_high],['15m Low',q.first15_low]].map(a=>`<div class="card"><div class="label">${esc(a[0])}</div><div class="value">${esc(a[1])}</div></div>`).join('');document.getElementById('progress').innerHTML=['Acquisition','29-stock validation','1m OHLC','5m OHLC','VWAP','EMA 9','EMA 20','Opening range','Swing structure'].map(x=>`<span class="stage">✓ ${x}</span>`).join('');document.getElementById('market').innerHTML=[['1m Open',q.open_1m],['1m High',q.high_1m],['1m Low',q.low_1m],['1m Close',q.close_1m],['5m Open',q.open_5m],['5m High',q.high_5m],['5m Low',q.low_5m],['5m Close',q.close_5m],['VWAP 5m',q.vwap_5m],['EMA 9 5m',q.ema9_5m],['EMA 20 5m',q.ema20_5m],['Swing High',q.swing_high],['Swing Low',q.swing_low],['First 15m High',q.first15_high],['First 15m Low',q.first15_low],['Timestamp',q.timestamp]].map(a=>cell(a[0],a[1])).join('');let sig=d.signal;if(!d.live_data){document.getElementById('signal').innerHTML='<div class="empty">Market closed. Deterministic data is for inspection only — <b>NOT LIVE</b>.</div>';return}if(!sig){document.getElementById('signal').innerHTML='<div class="empty">NO ACTIVE PSY29 SIGNAL</div>';return}document.getElementById('signal').innerHTML=`<div class="signal"><h3>${esc(sig.direction)} · ${esc(sig.strategy)}</h3><div class="signalrow">${[['Entry',sig.entry],['Stop Loss',sig.stop_loss],['Take Profit',sig.take_profit],['R:R',sig.risk_reward],['Score',sig.signal_score],['Status',sig.signal_status]].map(a=>`<div><div class="label">${esc(a[0])}</div><div class="value">${esc(a[1])}</div></div>`).join('')}</div></div>`}
...
class H(BaseHTTPRequestHandler):
 def do_GET(self):
  ...
  f=AV if p=="/live-validation" else PV if p=="/pipeline-validation" else None
