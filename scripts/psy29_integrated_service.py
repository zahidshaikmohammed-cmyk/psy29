...
def recent_ready():
 ...
 kinds={str(r.get("market_data_kind")) for r in rows}
 if kinds!={"MOST_RECENT_COMPLETED_NSE_SESSION"}:return False
...
def signal_data():
 ...
 recent_v=rj(RV,{}) or {}; recent_ok=recent_ready()
 ...
 if live:
  coverage=v.get("coverage",{"expected":29,"actual":0,"unique":0}); timestamp=b.get("timestamp") or v.get("timestamp"); kind="LIVE_DHAN"; session_date=None
 else:
  coverage=recent_v.get("coverage",{"expected":29,"actual":0,"unique":0}) if recent_ok else {"expected":29,"actual":0,"unique":0}
  timestamp=recent_v.get("generated_at") if recent_ok else None
  kind="MOST_RECENT_COMPLETED_NSE_SESSION" if recent_ok else "UNAVAILABLE"
  session_date=recent_v.get("session_date") if recent_ok else None
