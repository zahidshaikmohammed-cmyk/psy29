...
:root{color-scheme:dark;font-family:Inter,ui-sans-serif,system-ui,-apple-system,"Segoe UI",sans-serif;background:#111315;color:#edf0f2}*{box-sizing:border-box}body{margin:0;background:#111315}.app{display:grid;grid-template-columns:250px 1fr;min-height:100vh}.side{background:#0b0d0f;border-right:1px solid #252a2f;padding:16px;overflow:auto}.brand{font-size:18px;font-weight:750;padding:8px 10px 4px}.brand small{display:block;color:#737a81;font-size:10px;letter-spacing:.14em;text-transform:uppercase;margin-top:4px}.sub{font-size:10px;color:#737a81;letter-spacing:.12em;text-transform:uppercase;padding:18px 10px 8px}.stock{display:flex;width:100%;align-items:center;justify-content:space-between;border:0;background:transparent;border-radius:8px;padding:8px 10px;margin:1px 0;color:#d9dde1;text-align:left;cursor:pointer}.stock:hover,.stock.active{background:#20252a}.dot{width:6px;height:6px;border-radius:50%;background:#53c982;display:inline-block;margin-right:8px}.rank{color:#697078;font-size:10px}.main{padding:24px;max-width:1500px;width:100%;margin:auto}.top{display:flex;justify-content:space-between;gap:20px;align-items:flex-start}.eyebrow{font-size:10px;color:#7d858d;letter-spacing:.16em;text-transform:uppercase}.title{font-size:30px;font-weight:750;margin:4px 0}.muted{color:#858d95}.status{border:1px solid #30363d;background:#1a1f23;border-radius:18px;padding:8px 12px;font-size:11px;font-weight:650}.notice{margin-top:10px;padding:10px 12px;border:1px solid #30363d;border-radius:9px;background:#171b1f;color:#aeb5bb;font-size:11px}.hero{display:grid;grid-template-columns:1.4fr 1fr 1fr 1fr;gap:10px;margin:18px 0}.heroCard,.panel,.metric{background:#171b1f;border:1px solid #2a3036;border-radius:12px}.heroCard{padding:16px}.heroLabel{font-size:10px;color:#747c84;text-transform:uppercase;letter-spacing:.12em}.heroValue{font-size:22px;font-weight:750;margin-top:6px}.heroBig{font-size:28px}.section{margin-top:18px}.sectionTitle{display:flex;justify-content:space-between;align-items:center;margin-bottom:9px}.sectionTitle h2{font-size:13px;margin:0}.sectionTitle span{font-size:10px;color:#687078;text-transform:uppercase;letter-spacing:.1em}.chain{display:grid;grid-template-columns:repeat(5,1fr);gap:7px}.stage{background:#171b1f;border:1px solid #2a3036;border-radius:9px;padding:10px}.stageName{font-size:10px;color:#9da4aa;font-weight:650}.stageState{font-size:11px;margin-top:5px}.pass{color:#62d392}.final{border-color:#6a5d2c;background:#1d1b13}.final .stageState{color:#e2c35c}.opaque{color:#7e868d}.signal{padding:20px;background:#171b1f;border:1px solid #3a4148;border-radius:13px}.signalLive{border-color:#47795c;background:#141d18}.signalHead{display:flex;justify-content:space-between;align-items:center;gap:12px}.signalDir{font-size:26px;font-weight:800}.pill{padding:5px 9px;border-radius:14px;font-size:10px;font-weight:700;background:#22282d;color:#9ca5ad}.signalGrid{display:grid;grid-template-columns:repeat(6,1fr);gap:10px;margin-top:16px}.metric{padding:12px}.metricLabel{font-size:9px;color:#747c84;text-transform:uppercase;letter-spacing:.1em}.metricValue{font-size:18px;margin-top:5px;font-variant-numeric:tabular-nums}.empty{padding:28px;text-align:center;color:#929aa1;border:1px dashed #394047;border-radius:11px;background:#15191d}.marketGrid{display:grid;grid-template-columns:repeat(4,1fr);gap:7px}.kv{background:#15191d;border:1px solid #292f35;padding:10px;border-radius:8px}.k{font-size:9px;color:#747c84;text-transform:uppercase}.v{font-size:13px;margin-top:4px;font-variant-numeric:tabular-nums}.audit{display:grid;grid-template-columns:repeat(4,1fr);gap:7px}.audit .kv{min-height:55px}.foot{margin:22px 0 6px;color:#60686f;font-size:10px;text-align:center}@media(max-width:1000px){.hero{grid-template-columns:repeat(2,1fr)}.chain{grid-template-columns:repeat(3,1fr)}.signalGrid{grid-template-columns:repeat(3,1fr)}.marketGrid,.audit{grid-template-columns:repeat(2,1fr)}}@media(max-width:720px){.app{grid-template-columns:1fr}.side{max-height:220px;border-right:0;border-bottom:1px solid #252a2f}.hero,.chain,.signalGrid,.marketGrid,.audit{grid-template-columns:1fr 1fr}.main{padding:16px}.title{font-size:25px}}
</style></head><body><div class="app"><aside class="side"><div class="brand">PSY29 <small>Capital Control Tower</small></div><div class="sub">29 instrument universe</div><div id="stocks"></div></aside><main class="main"><div class="top"><div><div class="eyebrow">Read-only trading control tower</div><div class="title" id="title">Select instrument</div><div class="muted">Stage 20 remains the sole signal authority · no execution controls</div><div class="notice" id="notice">Loading DHAN provenance…</div></div><div class="status" id="status">CONNECTING</div></div><div class="hero"><div class="heroCard"><div class="heroLabel">Final signal authority</div><div class="heroValue heroBig" id="heroSignal">WAITING</div><div class="muted" id="heroSub">Awaiting live Stage 20 output</div></div><div class="heroCard"><div class="heroLabel">Market mode</div><div class="heroValue" id="heroMode">—</div><div class="muted" id="heroSession">—</div></div><div class="heroCard"><div class="heroLabel">Data integrity</div><div class="heroValue" id="heroCoverage">—</div><div class="muted">Canonical universe</div></div><div class="heroCard"><div class="heroLabel">Active signals</div><div class="heroValue" id="heroCount">0</div><div class="muted">Stage 20 output only</div></div></div><div class="section"><div class="sectionTitle"><h2>Signal dependency chain</h2><span>UI observability only · backend unchanged</span></div><div class="chain" id="chain"></div></div><div class="section"><div class="sectionTitle"><h2>Stage 20 final signal</h2><span>sole authority</span></div><div id="signal"></div></div><div class="section"><div class="sectionTitle"><h2>Selected market state</h2><span>DHAN source</span></div><div class="marketGrid" id="market"></div></div><div class="section"><div class="sectionTitle"><h2>Provenance & safety</h2><span>read-only</span></div><div class="audit" id="audit"></div></div><div class="foot">PSY29 Control Tower · presentation layer only · no signal logic added</div></main></div><script>
let selected=null,universe=[];const esc=x=>String(x??'—').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));const fmt=x=>{if(x===null||x===undefined||x==='')return '—';let n=Number(x);return Number.isFinite(n)?n.toLocaleString('en-IN',{maximumFractionDigits:2}):String(x)};async function j(u){return(await fetch(u,{cache:'no-store'})).json()}function kv(k,v){return `<div class="kv"><div class="k">${esc(k)}</div><div class="v">${esc(v)}</div></div>`}
function renderChain(s){const names=['Stage 6 · Regime','Stage 7 · Edge Activation','Stage 8 · Edge Ranking','Stage 9 · Candidate Quality','Stage 10 · Integrity','Stage 11 · Execution Analysis','Stage 12 · Readiness','Stage 13 · Scenario','Stage 14 · Dashboard','Stage 15 · Event Journal','Stage 16 · Decision Board','Stage 17 · Stability','Stage 18 · Continuity','Stage 19 · Transition','Stage 20 · FINAL AUTHORITY'];const dataReady=s.coverage&&s.coverage.expected===29&&s.coverage.actual===29&&s.coverage.unique===29;document.getElementById('chain').innerHTML=names.map((n,i)=>{let state=i===14?(s.live_data?(s.active_signal_count?'OUTPUT AVAILABLE':'NO ACTIVE SIGNAL'):'MARKET CLOSED'):i<2&&dataReady?'UPSTREAM DATA PASS':'ENGINE TELEMETRY NOT EXPOSED';let cls=i===14?'final':i<2&&dataReady?'':'opaque';return `<div class="stage ${cls}"><div class="stageName">${n}</div><div class="stageState ${i<2&&dataReady?'pass':i===14?'':'opaque'}">${esc(state)}</div></div>`}).join('')}
async function load(){let s=await j('/api/signals');document.getElementById('status').textContent=s.live_data?'● LIVE · DHAN':'OFF-MARKET · RECENT DHAN';document.getElementById('heroMode').textContent=s.live_data?'LIVE':'CLOSED';document.getElementById('heroSession').textContent=s.live_data?'Current NSE session':'Completed: '+(s.recent_session_date||'—');document.getElementById('heroCoverage').textContent=(s.coverage?.actual||0)+' / '+(s.coverage?.expected||29);document.getElementById('heroCount').textContent=s.active_signal_count||0;document.getElementById('heroSignal').textContent=s.live_data&&s.active_signal_count?'SIGNAL AVAILABLE':s.live_data?'NO SIGNAL':'MARKET CLOSED';document.getElementById('heroSub').textContent=s.live_data?'Stage 20 output only':'Recent DHAN data is display-only';document.getElementById('notice').textContent=s.live_data?'Live DHAN session data.':'Real DHAN historical data from the most recent completed NSE session ('+(s.recent_session_date||'pending')+'). Validation: '+(s.coverage?.actual||0)+'/'+(s.coverage?.expected||29)+'; fixture data is blocked.';renderChain(s);if(!universe.length){universe=(await j('/api/universe')).symbols||[];document.getElementById('stocks').innerHTML=universe.map((x,i)=>`<button class="stock" data-s="${esc(x)}" onclick="select('${esc(x)}')"><span><i class="dot"></i>${esc(x)}</span><span class="rank">#${i+1}</span></button>`).join('')}if(selected)await select(selected);else if(universe[0])await select(universe[0])}
async function select(x){selected=x;document.querySelectorAll('.stock').forEach(b=>b.classList.toggle('active',b.dataset.s===x));let d=await j('/api/instrument/'+encodeURIComponent(x));document.getElementById('title').textContent=x;if(!d.found){document.getElementById('signal').innerHTML='<div class="empty">'+esc(d.message)+'</div>';return}let q=d.data;document.getElementById('market').innerHTML=[['Last',fmt(q.close_1m)],['1m Open',fmt(q.open_1m)],['1m High',fmt(q.high_1m)],['1m Low',fmt(q.low_1m)],['5m Close',fmt(q.close_5m)],['VWAP',fmt(q.vwap_5m)],['EMA 9',fmt(q.ema9_5m)],['EMA 20',fmt(q.ema20_5m)],['5m Volume',fmt(q.volume_5m)],['15m High',fmt(q.first15_high)],['15m Low',fmt(q.first15_low)],['Swing High',fmt(q.swing_high)],['Swing Low',fmt(q.swing_low)],['Session',q.session_date],['Timestamp',q.timestamp],['Provenance',q.market_data_kind]].map(a=>kv(a[0],a[1])).join('');let sig=d.signal;if(!d.live_data){document.getElementById('signal').innerHTML='<div class="empty"><b>MARKET CLOSED</b><br><br>Real recent DHAN data is displayed for analysis only.<br>Signal generation is not shown as active. Fixture data is blocked.</div>';return}if(!sig){document.getElementById('signal').innerHTML='<div class="empty"><b>NO ACTIVE PSY29 SIGNAL</b><br><br>The existing Stage 20 board has no active signal for this instrument.</div>';return}document.getElementById('signal').innerHTML=`<div class="signal signalLive"><div class="signalHead"><div><div class="signalDir">${esc(sig.direction)} · ${esc(sig.strategy)}</div><div class="muted">${esc(sig.symbol)} · ${esc(sig.signal_id)}</div></div><div class="pill">${esc(sig.signal_status)}</div></div><div class="signalGrid">${[['Entry',fmt(sig.entry)],['Stop Loss',fmt(sig.stop_loss)],['Take Profit',fmt(sig.take_profit)],['R:R',fmt(sig.risk_reward)],['Score',fmt(sig.signal_score)],['Generated',sig.timestamp]].map(a=>`<div class="metric"><div class="metricLabel">${esc(a[0])}</div><div class="metricValue">${esc(a[1])}</div></div>`).join('')}</div></div>`}
