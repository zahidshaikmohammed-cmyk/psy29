...
def make_fixture() -> tuple[pd.DataFrame, datetime]:
    ...
    for i in range(30):
        ...
        price = 100.0 + i * 0.2
        rows.append({"timestamp": int(dt.timestamp()), "open": price, "high": price + 0.5,
                     "low": price - 0.5, "close": price + 0.2, "volume": 1000 + i})
    for i in range(17):
        dt = session + timedelta(minutes=i)
        price = 110.0 + i * 0.1
        rows.append({"timestamp": int(dt.timestamp()), "open": price,
                     "high": price + 0.4 + (0.1 if i == 4 else 0),
                     "low": price - 0.4 - (0.1 if i == 7 else 0),
                     "close": price + 0.1, "volume": 2000 + i * 10})
